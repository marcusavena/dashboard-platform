...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <h1 className="text-3xl font-bold mb-8">Frontier Capitald</h1>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-200">
            {deals.map((deal) => (
              <tr key={deal.id}>
                <td className="px-6 py-4">{deal.company_name}</td>
                <td className="px-6 py-4">${Number(deal.funded_amount).toLocaleString()}</td>
                <td className="px-6 py-4">{deal.rate}</td>
                <td className="px-6 py-4">{deal.term_days} days</td>
